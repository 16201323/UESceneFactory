...
class QualityGuardAgent(AgentBase):
    ...
    def _system_prompt(self) -> str:
        ...
        return (
            ...
            "🚫 修复原则（最重要，违反将导致场景渲染缺陷）：\n"
            "- 禁止通过删除字段来消除错误！校验器对 null/缺失/空字典 都会报 error，\n"
            "  删除字段不会让错误消失，只会让 build_scene.py 静默跳过该功能。\n"
            "- grass/height_pattern/lighting/weather 是必填字段，null 会导致：\n"
            "  灰色地形(无草) / 完全平坦(无高度) / 场景全黑(无光照) / 无云(无天气)\n"
            "- 遇到错误必须填充正确内容，绝不能删除该字段或置为 null。\n"
            "- grid 类型的 placement 缺 grid 时，必须补充 grid 配置，不能删除该条目。\n\n"
            "📝 内容丰富度修复（根因5 — 修复空壳场景）：\n"
            "- 当 quality_errors 报告以下空内容错误时, 必须根据蓝图意图生成实际内容:\n"
            "  • 'placements 列表为空' → 根据蓝图 placements 列表逐一生成,\n"
            "    每个条目含 type/asset/location 等完整字段\n"
            "  • 'layers 图层数组为空' → 至少生成 2 个图层(如 Grass+Dirt),\n"
            "    每个图层有 info(LayerInfo 路径) 和 weight(0~1)\n"
            "  • 'hills/valleys/ridges 全为空' → 根据 terrain_type 生成至少 1 个\n"
            "    地形特征(丘陵→hills, 山谷→valleys, 山脊→ridges)\n"
            "- 空数组 = 功能缺失, 不是'可选省略'。校验器会报 error 而非 warning。\n"
            "- 修复空内容时参考蓝图描述(如'北方农村'应生成房屋+树木+麦田放置物)。\n\n"
