...
class ScenePlannerAgent(AgentBase):
    ...
    def _system_prompt(self) -> str:
        return (
            ...
            "概念展开规则(按用户描述的关键词选择):\n"
            "- 农村/乡村/村庄 → 远景山脉背景+溪流+成片森林+村落房屋(必含 village 和 forest)\n"
            "  ⚠️ 房屋数量铁律: 村落必须有 5~15 栋房屋,绝不能只放 1-2 栋\n"
            "     JSON 中 village 放置条目需配置足够数量(grid rows×cols≥6 或多个 static 实例)\n"
            "  ⚠️ 远山铁律: 农村/山区场景的 height_pattern 必须配置 hills 或 ridges 作为远景山脉\n"
            "  例: terrain_type=features 或 hill_ridge, has_water=true, has_river=true,\n"
            "      has_grass=true, placements=[\"village\",\"forest\"],\n"
            "      keywords=[\"山\",\"树\",\"房屋\",\"河流\",\"草地\",\"远山\"]\n"
            ...
            "      keywords=[\"厂房\",\"电塔\",\"路\",\"光伏\"]\n"
            "未匹配以上时,默认按\"综合自然场景\"展开:远景山脉+河流+森林+草地+村落(5~15栋房屋)。\n\n"
            "==== 第1步:字段取值规则 ====\n"
            ...
            "⚠️ 房屋数量: placements 含 village 时,须在 user_desc 或 keywords 中体现\n"
            "   \"5~15栋房屋\"的数量提示,供 Stage2 JSONBuilder 配置足够数量的房屋实例。\n"
            "⚠️ 远景山脉: 除平原/沙漠/城市外,height_pattern 应含 hills 或 ridges\n"
            "   作为远景山脉背景。keywords 中应包含\"远山\"或\"山\"关键词。\n"
        )
